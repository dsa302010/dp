...
class FrogPilotEvents:
  ...
  def update(self, carState, controlsState, frogpilotCarControl, frogpilotCarState, lead_distance, modelData, v_lead, frogpilot_toggles):
    ...
    if not self.holiday_theme_played and self.startup_seen and controlsState.alertText1 == "" and frogpilot_toggles.current_holiday_theme != "stock":
    ...
    if not self.random_event_played and frogpilot_toggles.random_events:
      ...
      steer_saturated_alert_match = controlsState.alertText1 == EVENTS[EventName.steerSaturated][ET.WARNING].alert_text_1 and controlsState.alertText2 == EVENTS[EventName.steerSaturated][ET.WARNING].alert_text_2
      if steer_saturated_alert_match:
        event_choices = []
    ...
    if not self.nnff_played and self.startup_seen and controlsState.alertText1 == "" and params.get("NNFFModelName", encoding="utf-8") is not None:
